	...
	<main id="main" class="site-main row" role="main">
		...
		while ( have_posts() ) : the_post();
			...
			if ( is_singular( 'attachment' ) ) {
			    ...
			    the_post_navigation( array(
		        'next_text' => '<span class="meta-nav col-xs-12 col-sm-6" aria-hidden="true">' . __( 'Next', 'twentyfifteen' ) . '</span> ' .
		            '<span class="screen-reader-text">' . __( 'Next post:', 'twentyfifteen' ) . '</span> ' .
		            '<span class="post-title">%title</span>' . get_the_post_thumbnail($next_post->ID,'thumbnail'),
		        'prev_text' => '<span class="meta-nav col-xs-12 col-sm-6" aria-hidden="true">' . __( 'Previous', 'twentyfifteen' ) . '</span> ' .
